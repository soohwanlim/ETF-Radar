    ...
    <div className="p-5 rounded-2xl bg-slate-950/50 border border-slate-900 hover:border-slate-800 transition-colors space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <Link to={`/etf/${etf.code}`} className="font-bold text-slate-200 hover:text-blue-400 block">
            {etf.name}
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-wrap gap-1.5">
            {holdings.map(holding => (
              <span key={`${holding.code}-${holding.name}`} className="text-xs px-2.5 py-1 rounded-lg bg-slate-900 border border-slate-800 text-slate-300">
    ...
    <div className="space-y-8 fade-in">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 py-4">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight flex items-center gap-2 text-slate-100">
            <LayoutGrid className="text-emerald-400" /> 테마 핫 ETF
          </h1>
          <p className="text-slate-400 text-sm mt-2">국내 주식형 ETF를 산업 테마로 자동 분류하고, 상위 3개 ETF 평균 수익률로 정렬합니다.</p>
        </div>
        <div className="flex bg-slate-900/80 p-1 rounded-xl border border-slate-800">
          {PERIODS.map(item => (
            <button key={item.id} onClick={() => { setPeriod(item.id); setSelectedTheme(null); }}
              className={`px-4 py-2 text-xs font-bold rounded-lg transition-all ${period === item.id ? 'bg-emerald-500 text-slate-950' : 'text-slate-400 hover:text-slate-200'}`}>
              {item.label}
            </button>
          ))}
        </div>
      ...
      {loading ? (
        <div className="glass rounded-3xl p-20 flex items-center justify-center gap-3 text-slate-500 text-sm">
          <Loader2 className="animate-spin text-emerald-400" /> 테마 수익률 계산 중...
        </div>
      ) : error ? (
        <div className="glass rounded-3xl p-12 text-center text-rose-400 text-sm">{error}</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="space-y-3">
            <h2 className="text-lg font-bold text-slate-200 px-1">수익률 상위 테마</h2>
            {themes.map((theme, index) => {
              const Icon = theme.icon;
              const selected = theme.id === activeTheme?.id;
              return (
                <button key={theme.id} onClick={() => setSelectedTheme(theme.id)}
                  className={`w-full text-left p-4 rounded-2xl flex items-center justify-between border transition-all ${selected ? 'bg-gradient-to-r from-emerald-500/20 to-blue-500/10 border-emerald-500/40' : 'glass hover:border-slate-800'}`}>
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="text-xs font-bold text-slate-600 w-4">{index + 1}</span>
                    <div className={`p-2 rounded-xl ${selected ? 'bg-emerald-500 text-slate-950' : 'bg-slate-900 text-slate-400'}`}><Icon size={17} /></div>
                    <div className="min-w-0">
                      <div className="font-semibold text-slate-200 truncate">{theme.name}</div>
          ...
          <div className="md:col-span-2 glass p-6 rounded-3xl space-y-5">
            {activeTheme && (
              <>
                <div>
                  <span className="text-xs font-semibold text-emerald-400">HOT THEME</span>
                  <h2 className="text-2xl font-extrabold text-slate-100 mt-1">{activeTheme.name} 대표 ETF</h2>
                  <p className="text-xs text-slate-400 mt-1">{period === '1w' ? '최근 1주' : '최근 1개월'} 수익률이 높은 순서입니다.</p>
                </div>
