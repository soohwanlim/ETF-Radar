  ...
  return (
    <div className="glass p-6 rounded-3xl relative glow-blue space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4 text-xs font-mono border-y border-slate-200 py-4">
        ...
        <div><span className="text-[10px] text-slate-500 block">{etf.assetValueType === 'netAssets' ? '순자산' : '시가총액'}</span><span className="font-semibold text-slate-700 text-sm">{etf.aum == null ? '-' : `${etf.aum.toLocaleString()}억`}</span></div>
        <div><span className="text-[10px] text-slate-500 block">총보수</span><span className="font-semibold text-slate-700 text-sm">{etf.fee == null ? '-' : `${etf.fee}%`}</span></div>
        <div>
      ...
      <div>
        ...
        {loading ? <Loader2 className="animate-spin text-slate-600" size={14} /> : (
          <div className="flex flex-wrap gap-1">
            {holdings.map(holding => <span key={`${holding.code}-${holding.name}`} className="text-[11px] px-2 py-0.5 rounded bg-white text-slate-600">{holding.name} ({holding.value}%)</span>)}
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="space-y-8 fade-in">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 py-4">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight flex items-center gap-2 text-slate-950"><ArrowLeftRight className="text-blue-600" /> ETF 비교분석</h1>
          <p className="text-slate-600 text-sm mt-2">최대 4개 ETF의 기간 수익률과 구성종목을 비교합니다.</p>
        </div>
        {selectedEtfs.length > 0 && <button onClick={clearSelected} className="flex items-center gap-2 text-xs font-semibold px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl"><Trash2 size={14} /> 비교 목록 전체 삭제</button>}
      </div>

      {selectedEtfs.length === 0 ? (
        <div className="glass rounded-3xl p-16 text-center text-slate-600 space-y-4">
          <ArrowLeftRight size={48} className="text-slate-600 mx-auto" />
          <h2 className="text-lg font-bold text-slate-700">비교할 ETF가 없습니다.</h2>
          <p className="text-xs text-slate-500">메인 수익률 랭킹에서 비교할 ETF를 선택해 주세요.</p>
        </div>
      ) : (
        <>
          <div className="flex flex-wrap bg-white p-1 rounded-xl border border-slate-200 w-fit">
            {PERIODS.map(item => <button key={item.id} onClick={() => setPeriod(item.id)} className={`px-4 py-2 text-xs font-bold rounded-lg transition-all ${period === item.id ? 'bg-blue-500 text-white' : 'text-slate-600 hover:text-slate-900'}`}>{item.label}</button>)}
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
            {etfsLoading ? <div className="col-span-full py-16 flex justify-center"><Loader2 className="animate-spin text-blue-600" /></div> : selectedDetails.map(etf => <CompareCard key={etf.code} etf={etf} period={period} onRemove={() => removeEtf(etf.code)} />)}
          </div>

          <div className="glass p-6 rounded-3xl space-y-4">
            <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2"><TrendingUp size={18} className="text-blue-600" /> 선택 기간 누적수익률 비교</h2>
            <div className="h-[340px] w-full pt-4">
              {chartLoading ? (
                <div className="h-full flex items-center justify-center gap-2 text-slate-500 text-sm"><Loader2 className="animate-spin text-blue-600" /> 가격 시계열 계산 중...</div>
              ) : chartData.length > 1 ? (
