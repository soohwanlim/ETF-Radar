  ...
  return (
    <div className="space-y-8 fade-in">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 py-4">
        ...
        {watchlist.length > 0 && (
          <span className="text-xs px-3 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-600 font-semibold">
      ...
      {watchlist.length === 0 ? (
        <div className="glass rounded-3xl p-16 text-center space-y-4">
          ...
          <h2 className="text-lg font-bold text-slate-700">저장한 ETF가 없습니다.</h2>
          ...
          <div className="flex flex-wrap bg-white p-1 rounded-xl border border-slate-200 w-fit">
            {PERIODS.map(item => (
              <button
                ...
                className={`px-4 py-2 text-xs font-bold rounded-lg transition-all ${period === item.id ? 'bg-amber-500 text-slate-950' : 'text-slate-600 hover:text-slate-900'}`}
              >
                {item.label}
              </button>
            ))}
          ...
          <div className="glass rounded-2xl overflow-hidden overflow-x-auto glow-blue">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-200 text-sm">
                {loading ? (
                  <tr><td colSpan="5" className="py-20 text-center"><Loader2 className="animate-spin text-amber-600 mx-auto" /></td></tr>
                ) : error ? (
                  <tr><td colSpan="5" className="py-12 text-center text-rose-600 text-xs">{error}</td></tr>
                    ...
                    <tr key={etf.code} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-4 px-5">
                        <div className="flex items-center justify-center gap-2">
                          <button onClick={() => toggleWatchlist(etf.code)} className="p-1.5 rounded-lg bg-amber-500/10 border border-amber-500/30 text-amber-600" title="즐겨찾기 해제">
                            <Star size={14} fill="currentColor" />
                          </button>
                          <button
                            onClick={() => toggleCompare(etf.code)}
                            className={`text-xs px-2.5 py-1.5 rounded-lg font-semibold border transition-all ${selectedEtfs.includes(etf.code) ? 'bg-blue-600 border-blue-500 text-white' : 'border-slate-200 text-slate-600 hover:text-slate-900 hover:bg-slate-100'}`}
                          >
