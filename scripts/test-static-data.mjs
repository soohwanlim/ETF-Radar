...
import { getEtfIndexability, selectIndexableEtfs } from '../src/data/etfIndexing.js';
...
const indexableEtf = {
  ...
  provider: '테스트운용',
  asOf: '2026-09-10',
  description: '검색 사용자가 상품의 운용 목적과 기초지수 특징을 이해할 수 있도록 충분한 길이로 작성된 상품 설명입니다. 이 설명은 품질 기준 검증을 위한 테스트 데이터입니다.',
  rate1d: 1,
  rate1m: 2,
  rate1y: 3,
};
const indexableHoldings = Array.from({ length: 5 }, (_, index) => ({ code: `00${index}`, weight: 10 }));
assert.deepEqual(getEtfIndexability(indexableEtf, indexableHoldings), { indexable: true, reasons: [] });
assert.deepEqual(getEtfIndexability({ ...indexableEtf, provider: '', description: '' }, []), {
  indexable: false,
  reasons: ['provider', 'description', 'holdings'],
});
assert.deepEqual(selectIndexableEtfs([indexableEtf, { ...indexableEtf, code: 'B', rate1m: null, rate1y: null }], { A: indexableHoldings, B: indexableHoldings }), [indexableEtf]);
